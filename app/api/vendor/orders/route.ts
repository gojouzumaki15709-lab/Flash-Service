...
export async function GET() {
  ...
  const { data, error } = await db
    .from("orders")
    .select(
      "id, total, created_at, client_room, client:clients(name, phone), payment_method:payment_methods(type, label), items:order_items(id, quantity, unit_price, product:products(id, name))"
    )
