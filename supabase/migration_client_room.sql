-- ============================================================
-- migration_client_room.sql
--
-- À exécuter APRÈS migration_hardening_v4.sql.
--
-- Objectif : le client précise sa chambre (ex: "B-22") au moment
-- de CHAQUE commande (ce n'est pas un champ de profil fixe : un
-- même client peut commander depuis des endroits différents).
--
--   1) Ajoute orders.client_room (texte libre, ex: "B-22").
--   2) Recrée create_order_atomic() pour accepter et enregistrer
--      ce champ (nouveau paramètre p_client_room).
--
-- Idempotent : ADD COLUMN IF NOT EXISTS + CREATE OR REPLACE
-- supportent la ré-exécution. Le DROP FUNCTION cible explicitement
-- l'ancienne signature (6 paramètres) pour ne pas laisser un
-- doublon orphelin en base.
-- ============================================================

alter table orders add column if not exists client_room text;

-- Supprime l'ancienne signature (6 paramètres, sans p_client_room)
-- si elle existe encore, pour éviter d'avoir deux fonctions
-- create_order_atomic en parallèle en base.
drop function if exists create_order_atomic(uuid, uuid, jsonb, uuid, boolean, numeric);
...
create or replace function create_order_atomic(
  ...
  p_client_room text          -- chambre du client au moment de la commande, ex: "B-22"
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  -- Chambre obligatoire : nettoyée (espaces retirés), rejetée si vide.
